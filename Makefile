...
LINT                    = $(SPYGLASS_LINT)
LINT_FLAGS              = --top $(TOP_MODULE) --files $(subst $(TOP_DIR)/,,$(VERILOG_SRC)) --includes $(subst $(TOP_DIR)/,,$(RTL_PATHS)) --license $(RTL_LINTER_LICENSE) $(SPYGLASS_REMOTE_OPTION) $(LINTER_ENV_OPTION) $(SPYGLASS_WAIVER_OPTION) --move --debug
else
LINT                    = $(VERILATOR_LINT)
